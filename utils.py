...
def plot_train_result(real_image, gen_image, recon_image, epoch, save=False, save_dir='results/', show=False, fig_size=(5, 5)):
    ...
    if save:
        ...
        save_fn = save_dir + 'Result_epoch_{:d}'.format(epoch+1) + '.jpg'
        plt.savefig(save_fn)
...
def plot_test_result(real_image, gen_image, recon_image, index, save=False, save_dir='results/', show=False):
    ...
    if save:
        ...
        save_fn = save_dir + 'Test_result_{:d}'.format(index + 1) + '.jpg'
...
# only one test image
def save_singleimages(gen_image, index, save=False, save_dir='results/', show=False):
    fig_size = (256, 256)
    fig, axes = plt.subplots(1, 1, figsize=fig_size)

    imgs = to_np(gen_image)
    """
    for ax, img in zip(axes.flatten(), imgs):
        ax.axis('off')
        ax.set_adjustable('box')
    ...
    if save:
        ...
        save_fn = save_dir + 'single_{:d}'.format(index + 1) + '.jpg'
        fig.subplots_adjust(bottom=0)
        fig.subplots_adjust(top=1)
        fig.subplots_adjust(right=1)
        fig.subplots_adjust(left=0)
    ...
    for i in range(gen_image.size(0)):
        fullpath = save_dir + 'single_{:d}'.format(index + 1) + '.jpg'
        # range from [-1, 1] to [0, 1]
        # img = (images[i] + 1.0) / 2
        img = gen_image[i].add(1).div(2).mul(255).clamp(0, 255).byte()
